package dev.alejandroaitorjesusdiego.proyectoprogramacion3.routes
...
import dev.alejandroaitorjesusdiego.proyectoprogramacion3.gestion.controllers.SplashController
...
object RoutesManager {
     ...
     * @property JORNADA Vista de la plantilla de la proxima jornada
     */
    ...
    fun initAdminStage(stage: Stage) {
        ...
        stage.icons.add(Image(getResourceAsStream("media/logoNuevo.png")))
        stage.scene = scene
    ...
    /**
     * Inicia la vista de proxima jornada
     * @param stage [Stage] vista de proxima jornada
     * @see [Vistas.JORNADA]
     */

    fun initJornadaStage() {
        logger.debug { "Iniciando jornadas" }
        val fxmlLoader = FXMLLoader(getResource(Vistas.JORNADA.path))
        val parentRoot = fxmlLoader.load<Pane>()
        val myScene = Scene(parentRoot)
        val stage = Stage()
        stage.title = "Proximo Partido"
        stage.scene = myScene
        stage.initOwner(mainStage)
        stage.centerOnScreen()
        stage.isIconified = false
        stage.initModality(Modality.WINDOW_MODAL)
        stage.icons.add(Image(getResourceAsStream("media/plantilla.png")))
        stage.isResizable = false
        stage.sizeToScene()
        stage.show()
    }
